...
def obtener_cargos_por_categoria(db: Session, id_categoria: int):
    cargos = (
        db.query(CargoOfrecido).filter(CargoOfrecido.id_categoria == id_categoria).all()
    )
    if not cargos:
...
def crear_cargo_ofrecido(db: Session, cargo_data: CargoOfrecidoCreate):
    ...
    existing_cargo = (
        db.query(CargoOfrecido)
        .filter(CargoOfrecido.nombre_cargo == cargo_data.nombre_cargo)
        .first()
    )
    if existing_cargo:
