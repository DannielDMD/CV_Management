...
class CandidatoConocimiento(Base):
    ...
    id_candidato = Column(Integer, ForeignKey("candidatos.id_candidato", ondelete="CASCADE"), nullable=False)
    tipo_conocimiento = Column(String(50), nullable=False)
    id_habilidad_blanda = Column(Integer, ForeignKey("habilidades_blandas.id_habilidad_blanda", ondelete="CASCADE"), nullable=True)
    id_habilidad_tecnica = Column(Integer, ForeignKey("habilidades_tecnicas.id_habilidad_tecnica", ondelete="CASCADE"), nullable=True)
    id_herramienta = Column(Integer, ForeignKey("herramientas.id_herramienta", ondelete="CASCADE"), nullable=True)

    # Restricción de tipo_conocimiento dentro de __table_args__
    __table_args__ = (
        CheckConstraint("tipo_conocimiento IN ('blanda', 'tecnica', 'herramienta')", name="chk_tipo_conocimiento"),
    )
